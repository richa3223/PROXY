...
resource "aws_subnet" "public" {
  ...
  cidr_block = var.public_subnet_ipv6_native ? null : element(concat(var.public_subnets, [
    ""
  ]), count.index)
...
locals {
  vpc_endpoint_s3_id = var.enable_s3_endpoint == 1 ? aws_vpc_endpoint.s3[0].id : ""
}
...
resource "aws_vpc_endpoint" "s3" {
  ...
  tags         = merge({ "Name" = "s3-${var.name}" }, var.global_tags)
}

resource "aws_vpc_endpoint_route_table_association" "public_s3" {
  count           = local.create_public_subnets && (!var.one_nat_gateway_per_az || local.len_public_subnets >= length(var.azs)) ? local.len_public_subnets * var.associate_s3_endpoint_with_public_route_table : 0
  vpc_endpoint_id = local.vpc_endpoint_s3_id
  route_table_id  = aws_route_table.public[0].id
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone               = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id            = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
  cidr_block = var.private_subnet_ipv6_native ? null : element(concat(var.private_subnets, [
    ""
  ]), count.index)
...
resource "aws_vpc_endpoint_route_table_association" "private_s3" {
  count           = local.create_private_subnets && local.max_subnet_length > 0 ? local.nat_gateway_count * var.associate_s3_endpoint_with_private_route_table : 0
  vpc_endpoint_id = local.vpc_endpoint_s3_id
